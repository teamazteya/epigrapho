  ...
  if (result.totalNotes <= 0) {
    return (
      <Flex sx={{ flexDirection: "column", alignItems: "stretch" }}>
        <Text variant="title">{strings.importerUnsuccessful()}</Text>
        <Text variant="body" sx={{ mt: 2 }}>
          {strings.importerFailedDesc()}
        </Text>
  ...
  return (
    <>
      ...
      <Text variant="body" my={2} sx={{ textAlign: "center" }}>
        {strings.importCompleted()}.{" "}
        {strings.importerNotesImported(props.result.totalNotes)}{" "}
